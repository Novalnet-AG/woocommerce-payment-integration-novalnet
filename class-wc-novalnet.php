...
final class WC_Novalnet {
	 ...
	 public function add_epc_qr_to_order_emails( $fields, $sent_to_admin, $order ) {
		...
		if (!empty($qr_url)) {
				$fields['epc_qr'] = array(
					...
					'value' => '<img src="' . esc_url( $qr_url ) . '" alt="' . esc_attr( 'EPC QR Code' ) . '" style="' . esc_attr( 'max-width:220px;height:auto;margin:10px auto;display:block;' ) . '" />',
				);
			}
    ...
    public function add_epc_qr_to_order_success_page( $order ) {
		...
		if (!empty($qr_url)) {
			...
			echo '<img src="' . esc_url( $qr_url ) . '" alt="' . esc_attr( 'EPC QR Code' ) . '" style="' . esc_attr( 'max-width:220px;height:auto;margin:10px auto;display:block;' ) . '" />';
			echo '</section>';
		}
    }
