...
final class WC_Novalnet {
	...
	public function novalnet_shipping_address_update() {
		...
		if ( in_array( novalnet()->request['source_page'], array( 'mini_cart_page_googlepay_button', 'mini_cart_page_applepay_button', 'product_page_googlepay_button', 'product_page_applepay_button' ), true ) ) {
			if ( empty( novalnet()->request['variable_variant_id'] ) ) {
				...
				if ( isset( novalnet()->request['simple_product_id'] ) && ! in_array( (int) novalnet()->request['simple_product_id'], $product_id, true ) ) {
			...
			if ( ! empty( novalnet()->request['variable_product_id'] ) && ! empty( novalnet()->request['variable_variant_id'] ) ) {
				...
				if ( isset( novalnet()->request['variable_variant_id'] ) && ! in_array( (int) novalnet()->request['variable_variant_id'], $variation_id, true ) ) {
	...
	public function novalnet_order_creation() {
		...
		if ( isset( novalnet()->request['pay_for_order_id'] ) && ! empty( novalnet()->request['pay_for_order_id'] ) ) {
			...
			if ( isset( novalnet()->request['is_virtual_order'] ) && 'applepay' === (string) novalnet()->request['payment'] && 1 === (int) novalnet()->request['is_virtual_order'] ) {
			...
			foreach ( $required_fields as $key ) {
				if ( 'addressLines' === $key ) {
					...
					if ( isset( novalnet()->request['variable_name']['response']['order']['shipping']['contact'] ) && empty( trim( novalnet()->request['variable_name']['response']['order']['shipping']['contact'][ $key ], ' ' ) ) ) {
						$address        = 'Shipping';
						$missing_fields = true;
					}
				} else {
					if ( 'email' !== $key && empty( trim( novalnet()->request['variable_name']['response']['order']['billing']['contact'][ $key ], ' ' ) ) ) {
						$address        = 'Billing';
						$missing_fields = true;
					}
					if ( isset( novalnet()->request['variable_name']['response']['order']['shipping']['contact'] ) && empty( trim( novalnet()->request['variable_name']['response']['order']['shipping']['contact'][ $key ], ' ' ) ) ) {
