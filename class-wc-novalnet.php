...
final class WC_Novalnet
{
    ...
    public function novalnet_shipping_address_update()
    {
        ...
        foreach ($items as $item => $values) {
            ...
            $product_details = $_product->get_name() . ' (' .
                $values['quantity'] . ' X ' .
                $currency . number_format($product_price, 2) . ')';
    ...
    public function novalnet_order_creation()
    {
        ...
        if (isset($this->request['pay_for_order_id']) && ! empty($this->request['pay_for_order_id'])) {
            ...
            foreach ($required_fields as $key) {
                if ('addressLines' === $key) {
                    if (empty(trim($this->request['billing'][ $key ], ' '))) {
                        $address        = 'Billing';
                        $missing_fields = true;
                    }
                    if (isset($this->request['shipping']) && empty(trim($this->request['shipping'][ $key ], ' '))) {
                        $address        = 'Shipping';
                        $missing_fields = true;
                    }
                } else {
                    if ('email' !== $key && empty(trim($this->request['billing'][ $key ], ' '))) {
                        $address        = 'Billing';
                        $missing_fields = true;
                    }
                    if (isset($this->request['shipping']) && empty(trim($this->request['shipping'][ $key ], ' '))) {
        ...
        if (is_user_logged_in()) {
            ...
            WC()->customer->set_billing_last_name(wc_clean($customer_billing['last_name']));
            WC()->customer->set_billing_address_1(wc_clean((! empty($this->request['billing']['addressLines'])) ? $this->request['billing']['addressLines'] : WC()->customer->get_billing_address_1()));
            WC()->customer->set_billing_city(wc_clean($this->request['billing']['locality']));
            WC()->customer->set_billing_postcode(wc_clean($this->request['billing']['postalCode']));
            WC()->customer->set_billing_state(wc_clean($this->request['billing']['administrativeArea']));
            WC()->customer->set_billing_country(wc_clean($this->request['billing']['countryCode']));
            WC()->customer->set_billing_phone(wc_clean($customer_billing['phone']));
            $customer_billing['company'] = WC()->customer->get_billing_company();
            $customer_billing['state']   = WC()->customer->get_billing_state();
            if (isset($customer_shipping['first_name']) || isset($customer_shipping['last_name'])) {
                // Update shipping address from Applepay sheet.
                WC()->customer->set_shipping_first_name(wc_clean($customer_shipping['first_name']));
                WC()->customer->set_shipping_last_name(wc_clean($customer_shipping['last_name']));
                WC()->customer->set_shipping_address_1(wc_clean((! empty($this->request['shipping']['addressLines'])) ? $this->request['shipping']['addressLines'] : WC()->customer->get_billing_address_1()));
                WC()->customer->set_shipping_city(wc_clean($this->request['shipping']['locality']));
                WC()->customer->set_shipping_postcode(wc_clean($this->request['shipping']['postalCode']));
                WC()->customer->set_shipping_country(wc_clean($this->request['shipping']['countryCode']));
    ...
    {
        $sri = array(
            'woocommerce-novalnet-gateway-script'        => 'sha384-DLXJ1wIJtSjVtI3WbN1uMW14GoC2rq/h8VbGV2bYXcDDf4xaeeA2aaV9Ts6T8TVk',
