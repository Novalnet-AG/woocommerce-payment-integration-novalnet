...
final class WC_Novalnet {
	...
	public function __construct() {
		...
		$this->request = $_REQUEST; // phpcs ignore: WordPress.CSRF.NonceVerification.
	...
	public function novalnet_shipping_address_update() {
		...
		if ( in_array( novalnet()->request['source_page'], array( 'mini_cart_page_googlepay_button', 'mini_cart_page_applepay_button', 'product_page_googlepay_button', 'product_page_applepay_button' ), true ) ) {
			if ( empty( novalnet()->request['variable_variant_id'] ) ) {
				...
				if ( ! in_array( (int)novalnet()->request['simple_product_id'], $product_id ) ) {
		...
		if ( WC()->cart->show_shipping() && ( class_exists( 'WC_Subscriptions_Cart' ) && WC_Subscriptions_Cart::cart_contains_subscriptions_needing_shipping() ) ) {
			WC_Subscriptions_Cart::calculate_subscription_totals( WC()->cart->recurring_carts->total, WC()->cart->recurring_carts );
	...
	public function novalnet_order_creation() {
		...
		if ( is_user_logged_in() ) {
			...
			WC()->customer->set_billing_phone( wc_clean( $received_address['billing']['contact']['phoneNumber'] ) );
		...
		$customer_billing['phone']      = $received_address['billing']['contact']['phoneNumber'];
		...
		if ( $cart_has_subs ) {
			if ( is_user_logged_in() ) {
				if ( ! class_exists( 'WC_Subscriptions_Checkout' ) ) {
					...
					WC_Subscriptions_Checkout::process_checkout( $wc_order, $_POST ); // phpcs:ignore WordPress.Security.NonceVerification.
				}
			} else {
		...
		$result             = $available_gateways[ $payment_method ]->process_payment( $wc_order->id );
		wp_send_json( $result );
	}
