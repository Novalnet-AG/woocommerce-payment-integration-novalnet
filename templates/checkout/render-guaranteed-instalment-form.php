...
if ( WC_Novalnet_Validation::check_string( $payment_type, 'instalment' ) ) :
	...
	$instalment_details = apply_filters( 'novalnet_get_instalment_cycles', $contents, WC()->cart->total );
	...
	if ( ! empty( $instalment_details ['period'] ) ) :
		...
		woocommerce_form_field(
			...
			array(
				...
				'label'             => sprintf( __( 'Choose your instalment plan <b>(Net loan amount: %s )</b>', 'woocommerce-novalnet-gateway' ), wc_novalnet_shop_amount_format( WC()->cart->total * 100 ) ),
		...
		if ( ! empty( $instalment_details ['attributes'] ) ) :
			...
			foreach ( $instalment_details ['attributes'] as $period => $attributes ) :
				...
				$cycle_amount   = sprintf( '%0.2f', WC()->cart->total / $period );
				$splited_amount = $cycle_amount * ( $period - 1 );

				if ( $cycle_amount !== $splited_amount ) :
					$diff_amount = wc_novalnet_shop_amount_format( sprintf( '%0.2f', WC()->cart->total - $splited_amount ) * 100 );
				endif;
				?>
				<table id="<?php echo esc_html( $payment_type . '_table_' . $period ); ?>" class="shop_table woocommerce-checkout-review-order-table" style="display:none;">
					<tr>
