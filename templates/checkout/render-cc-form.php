...
<?php
	...
	// Enqueue script.
	wp_enqueue_script( 'woocommerce-novalnet-gateway-cc-script', novalnet()->plugin_url . '/assets/js/novalnet-cc.js', array( 'jquery', 'jquery-payment' ), NOVALNET_VERSION, false );
	wp_localize_script(
		'woocommerce-novalnet-gateway-cc-script',
		'wc_novalnet_cc_data',
		array(
			...
			'first_name'     => WC()->session->customer['first_name'],
			'last_name'      => WC()->session->customer['last_name'],
			'street'         => WC()->session->customer['address'],
			'city'           => WC()->session->customer['city'],
			'zip'            => WC()->session->customer['postcode'],
			'country_code'   => WC()->session->customer['country'],
			'email'          => WC()->session->customer['email'],
			'tel'            => WC()->session->customer['phone'],
		)
	);
	?>
