...
foreach ( $contents['available_wallets'] as $wallet ) {
	...
	$payment_setting      = WC_Novalnet_Configuration::get_payment_settings( 'novalnet_' . $wallet );
	...
	if ( $wallet_sheet_details['cart_has_subs'] <= 1 || 'yes' === get_option( 'novalnet_enable_shop_subs' ) ) {
		...
		if ( 'product_page' === $contents['wallet_area'] ) {
			echo '<input type = "hidden" id = "novalnet_product_id" value = "' . $wallet_sheet_details['add_product'] . '">';
			echo '<input type = "hidden" id = "product_has_virtual_product" value = "' . $wallet_sheet_details['cart_has_virtual'] . '"';
		}

		echo '
			<input type = "hidden" id = "cart_has_virtual" value = "' . $wallet_sheet_details['cart_has_virtual'] . '">
			<input type = "hidden" id = "wallet_area" value = "' . $contents['wallet_area'] . '">
			<input type = "hidden" id = "setpending" value = "' . $setpending . '">
			<input type = "hidden" id = "pay_for_order" value = "' . $wallet_sheet_details['pay_for_order'] . '">
			<input type = "hidden" id = "pay_for_order_id" value = "' . $wallet_sheet_details['pay_for_order_id'] . '">
			<input type = "hidden" id = "cart_has_one_time_shipping" value = "' . $wallet_sheet_details['cart_has_one_time_shipping'] . '">
			...
			<div data-type="cart" style="margin: 7px 0px" data-storeName="data-storeName" data-storeLang="' . wc_novalnet_shop_wallet_language() . '" data-total="' . (string) ( $wallet_sheet_details['cart_total'] * 100 ) . '" data-currency="' . get_woocommerce_currency() . '" data-country="' . $wallet_sheet_details['default_country'] . '" data-shopname="' . $wallet_sheet_details['seller_name'] . '"   data-id="' . $data_id . '" id="' . $wallet_id . '"></div>';
