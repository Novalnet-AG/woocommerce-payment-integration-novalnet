				...
				if ( ! empty( $items ) ) {
					foreach ( $items as $values ) {
						...
						if ( ( class_exists( 'WC_Subscriptions_Product' ) && WC_Subscriptions_Product::is_subscription( $product ) ) ) {
							if ( ( ! empty( $values['quantity'] ) && ( ( WC_Subscriptions_Product::get_price( $product ) * (int) $values['quantity'] ) * 100 ) < $settings['min_amount'] ) || wc_novalnet_formatted_amount( WC()->cart->total ) < $settings ['min_amount'] ) {
								return false;
							}
						}
					}
