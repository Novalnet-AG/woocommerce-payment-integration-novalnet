...
class WC_Novalnet_Validation {
		...
		if ( ! is_admin() && ! empty( $settings ['min_amount'] ) ) {
			if ( apply_filters( 'novalnet_cart_contains_subscription', true ) && ! is_account_page() ) {
				...
				if ( ! empty( $items ) ) {
					foreach ( $items as $values ) {
						...
						if ( ( class_exists( 'WC_Subscriptions_Product' ) && WC_Subscriptions_Product::is_subscription( $product ) ) ) {
							if ( ( ! empty( $values['quantity'] ) && ( ( WC_Subscriptions_Product::get_price( $product ) * (int) $values['quantity'] + WC()->cart->get_shipping_total() ) * 100 ) < $settings['min_amount'] ) || wc_novalnet_formatted_amount( WC()->cart->total ) < $settings ['min_amount'] ) {
								return false;
							}
						}
					}
				...
				if ( is_object( WC()->cart ) ) {
					$order_amount = wc_novalnet_formatted_amount( WC()->cart->total );
				}
	...
	public static function has_valid_guarantee_input( $session, $payment_type, $settings = array() ) {
		...
		} elseif ( ! WC()->session->__isset( $payment_type . '_dob_hided' ) ) {
