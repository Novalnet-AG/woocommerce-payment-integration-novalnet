...
class WC_Novalnet_Subscription {
	...
	public function reactivate_subscription_process( $can_update, $wcs_order ) {
		...
		// Checks Novalnet payment.
		if ( $can_update && WC_Novalnet_Validation::check_string( $wcs_order->get_payment_method() ) && ! get_post_meta( $wcs_order->get_id(), '_nn_subscription_updated', true ) && $this->check_subscription_status( 'active', 'on-hold' ) ) {
