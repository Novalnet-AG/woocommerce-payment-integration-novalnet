...
class WC_Novalnet_Subscription {
	...
	public function replace_nn_failed_order_payment_url( $pay_url, $wc_order ) {
		...
		if (
			...
			get_post_meta( $wc_order->get_id(), 'nn_failed_renewal', true ) &&
	...
	public function handle_admin_payment_process( $payment_type, $post_meta ) {
		if (
			( ! empty( novalnet()->request ['post_type'] ) && 'shop_subscription' === novalnet()->request ['post_type'] ) &&
			...
			if ( ( get_post_meta( novalnet()->request ['post_ID'], '_payment_method', true ) !== $payment_type ) || $new_payment_data ) {
