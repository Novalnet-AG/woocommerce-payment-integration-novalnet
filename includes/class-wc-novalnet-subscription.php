...
class WC_Novalnet_Subscription {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_subscription_payment_meta_input_novalnet_ach_post_meta_novalnet_ach_fields', array( &$this, 'novalnet_subscription_ach_fields' ), 10, 4 );
		...
		if (
			...
			if ( ! empty( $post_meta['post_meta']['novalnet_payments']['value'] ) ) {
				...
				if ( 'novalnet_sepa' === $payment_type ) {
					...
					$data['novalnet_ach_holder']  = novalnet()->request['novalnet_ach_holder'];
					$data['novalnet_ach_account'] = novalnet()->request['novalnet_ach_account'];
					$data['novalnet_ach_routing'] = novalnet()->request['novalnet_ach_routing'];
	 ...
	 * @since 12.7.0
	 */
	public function novalnet_subscription_ach_fields( $subscription, $field_id, $field_value, $meta_data ) {
		$settings = WC_Novalnet_Configuration::get_payment_settings( 'novalnet_ach' );
		if ( wc_novalnet_check_isset( $settings, 'enabled', 'yes' ) ) {
			// Display form fields.
	...
	public static function add_novalnet_payment_meta_details( $payment_meta ) {
		...
		$payment_meta['novalnet_ach']['post_meta']['novalnet_ach_fields'] = array(
			'value' => '',
			'label' => '     ',
		);
	...
	public function update_recurring_payment( $server_response, $wc_order_id, $payment_type, $wcs_order, $is_admin_change_payment = false ) {
		...
		if ( ( ! empty( $subs_tid ) && WC_Novalnet_Validation::check_string( $wcs_order->get_meta( '_old_payment_method' ) ) ) || ( ! empty( $subs_order_no ) ) ) {
			...
			if ( in_array( $payment_type, array( 'novalnet_sepa', 'novalnet_cc', 'novalnet_paypal', 'novalnet_guaranteed_sepa', 'novalnet_applepay', 'novalnet_googlepay', 'novalnet_ach' ), true ) ) {
				if ( ! empty( $server_response['transaction']['payment_data']['token'] ) ) {
