		...
		if ( ! empty( $tid ) ) {
			...
			if ( WC_Novalnet_Validation::is_success_status( $server_response ) ) {
				...
				if ( ! empty( $next_payment_date ) ) {
					...
					novalnet()->helper()->novalnet_update_wc_order_meta( $wcs_order, '_schedule_next_payment', gmdate( 'Y-m-d H:i:s', strtotime( $next_payment_date ) ) );
				}
	...
	public function get_subscription_supports( $supports, $payment_type ) {
		...
		if ( 'yes' === WC_Novalnet_Configuration::get_global_settings( 'enable_subs' ) ) {
			...
			if ( in_array( $payment_type, $subs_payments, true ) ) {
				...
				if ( ! in_array( $payment_type, array( 'novalnet_guaranteed_invoice', 'novalnet_guaranteed_sepa', 'novalnet_applepay', 'novalnet_googlepay' ), true ) ) {
