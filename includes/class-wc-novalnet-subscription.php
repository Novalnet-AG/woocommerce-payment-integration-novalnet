...
class WC_Novalnet_Subscription {
	...
	public function admin_enqueue_scripts() {
		...
		if ( isset( $post_type ) && 'shop_subscription' === $post_type ) {
			...
			if ( ! empty( novalnet()->request ['post'] ) ) {
				...
				if ( WC_Novalnet_Validation::check_string( $get_payment_method ) ) {
	...
	public function perform_subscription_recurring_amount_update( $wcs_order_id ) {
		...
		if ( wc_novalnet_check_isset( novalnet()->request, 'action', 'woocommerce_save_order_items' ) && 'shop_subscription' === get_post_type( $wcs_order_id ) ) {
	...
	public function update_next_payment_date( $wcs_order_id ) {
		...
		if ( WC_Novalnet_Validation::is_subscription_plugin_available() ) {
			...
			if ( 'shop_subscription' === $post_type && WC_Novalnet_Validation::check_string( $wcs_order->get_payment_method() ) && ! empty( novalnet()->request ['next_payment_timestamp_utc'] ) ) {
	...
	public function cancel_subscription_process( $can_update, $wcs_order ) {
		...
		if ( $can_update && ! WC_Novalnet_Validation::check_string( $wcs_order->get_status(), 'cancel' ) && WC_Novalnet_Validation::check_string( $wcs_order->get_payment_method() ) && ! get_post_meta( $wcs_order->get_id(), '_nn_subscription_updated', true ) && ( $this->check_subscription_status( 'cancel' ) || ! empty( novalnet()->request['novalnet_subscription_cancel_reason'] ) ) ) {
	...
	public function suspend_subscription_process( $can_update, $wcs_order ) {
		...
		if ( $can_update && WC_Novalnet_Validation::check_string( $wcs_order->get_payment_method() ) && ! get_post_meta( $wcs_order->get_id(), '_nn_subscription_updated', true ) && $this->check_subscription_status( 'on-hold', 'active' ) ) {
	...
	public function reactivate_subscription_process( $can_update, $wcs_order ) {
		...
		// Checks Novalnet payment.
		if ( $can_update && WC_Novalnet_Validation::check_string( $wcs_order->get_payment_method() ) && ( ! get_post_meta( $wcs_order->get_id(), '_nn_subscription_updated', true ) && $this->check_subscription_status( 'active', 'on-hold' ) ) || ( $this->check_subscription_status( 'active', 'cancelled' ) ) ) {
			$parameters = array();
		...
		} elseif ( $wcs_order->has_status( array( 'cancelled', 'expired' ) ) ) {
			return true;
		}
		return $can_update;
	}
	...
	public function customize_admin_subscription_process( $actions, $subscription ) {
		...
		if ( WC_Novalnet_Validation::check_string( $subscription->get_payment_method() ) && 'wc-pending-cancel' !== $subscription->get_status() ) {
	...
	public function handle_admin_payment_process( $payment_type, $post_meta ) {
		...
		if ( ! empty( $post_meta['post_meta']['novalnet_payment']['value'] ) ) {
			if ( ! empty( $recurring_payment_type ) && WC_Novalnet_Validation::check_string( $recurring_payment_type ) ) {
				$parameters = array(
			...
			if ( wc_novalnet_check_isset( $settings, 'enabled', 'yes' ) ) {
				...
				wc_enqueue_js(
                    ...
                    jQuery( '.edit_address' ).on( 'click', function( evt ) { 
                        ...
                        ( 'novalnet_cc' === jQuery( '#_payment_method option:selected' ).val() ) ? jQuery( '#novalnet-admin-psd2-notification' ).show() : jQuery( '#novalnet-admin-psd2-notification' ).hide();
                    } );
	...
	public function generate_subscription_parameters( $parameters, $wc_order, $is_change_payment = true ) {
		...
		if ( $this->is_subscription( $wc_order ) || $is_change_payment ) {
	...
	public function get_subscription_supports( $supports, $payment_type ) {
		...
		if ( 'yes' === WC_Novalnet_Configuration::get_global_settings( 'enable_subs' ) ) {
			...
			if ( in_array( $payment_type, $subs_payments, true ) ) {
				...
				if ( ( ! in_array( $payment_type, array( 'novalnet_guaranteed_invoice', 'novalnet_guaranteed_sepa', 'novalnet_paypal' ), true ) ) ) {
	...
	public function perform_subscription_post_process( $wc_order_id, $payment, $server_response, $wc_order ) {
		...
		if ( $this->is_subscription( $wc_order ) && empty( $server_response['event'] ['type'] ) ) {
			...
			if ( ! empty( $used_coupons ) ) {
				foreach ( $used_coupons as $coupon_code ) {
					...
					if ( 'recurring_fee' === $coupon->get_discount_type() ) {
						...
						foreach ( $meta_data_obj as $wc_meta_data ) {
							...
							if ( in_array( '_wcs_number_payments', $meta_data, true ) ) {
								update_post_meta( $wc_order_id, '_novalnet_wcs_number_payments', $meta_data['value'] );
							}
						}
					}
				}
			...
			novalnet()->db()->insert(
				array(
					...
					'recurring_tid'          => $server_response ['subscription']['tid'],
	...
	public function update_recurring_payment( $server_response, $wc_order_id, $payment_type, $wcs_order ) {
		...
		novalnet()->db()->update(
			array(
				...
				'recurring_tid'          => $server_response ['transaction']['tid'],
			),
			array(
				'order_no' => $wc_order_id,
			),
