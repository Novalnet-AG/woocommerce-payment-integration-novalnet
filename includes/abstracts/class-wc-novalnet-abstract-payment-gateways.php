...
abstract class WC_Novalnet_Abstract_Payment_Gateways extends WC_Payment_Gateway {
	...
	public function transaction_failure( $server_response, $wc_order, $is_webhook = false ) {
		...
		if( 'novalnet_applepay' == $this->id  ) {
			return $this->novalnet_redirect( $server_response['result']['status_text'], 'error');
		}
	...
	public function handle_order_id_update($wc_order, &$novalnet_response)
	{
		...
		{
			$novalnet_response['transaction']['invoice_ref'] = $new_invoice_ref = "BNR-{$novalnet_response['merchant']['project']}-{$wc_order->get_id()}";
			$parameters['transaction'] += array( 'invoice_no' => $wc_order->get_id(), 'invoice_ref' => $new_invoice_ref);
		}
