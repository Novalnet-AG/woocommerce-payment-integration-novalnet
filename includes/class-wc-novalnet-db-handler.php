...
class WC_Novalnet_DB_Handler
{
    ...
    public function get_subs_data_by_order_id($post_id, $subscription_id, $column = 'tid', $use_parent_id = true)
    {
        ...
        if (! empty($post_id) && in_array($column, array( 'tid', 'nn_txn_token', 'shop_based_subs', 'recurring_tid', 'subs_order_no', 'subs_id' ), true)) {
            ...
            $wpdb->my_custom_table = $wpdb->prefix . 'novalnet_subscription_details';
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $result = $this->handle_query($wpdb->get_var($wpdb->prepare("SELECT %i FROM %i WHERE order_no=%s AND subs_order_no=%s", $column, $wpdb->my_custom_table, $post_id, $subscription_id))); // db call ok; no-cache ok.
            ...
            if (empty($result) && $use_parent_id) {
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                $result = $this->handle_query($wpdb->get_var($wpdb->prepare("SELECT  %i FROM  %i WHERE order_no=%s", $column, $wpdb->my_custom_table, $post_id))); // db call ok; no-cache ok.
            }
        }
        return $result;
    }
