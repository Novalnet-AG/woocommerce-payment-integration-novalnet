...
class WC_Novalnet_DB_Handler {
	 ...
	 * @return array
	 */
	public function get_subs_data_by_order_id( $post_id, $subscription_id, $column = 'tid' ) {
		global $wpdb;

		$result = array();
		// Select transaction details based on post_id.
		if ( ! empty( $post_id ) && in_array( $column, array( 'tid', 'nn_txn_token', 'shop_based_subs', 'recurring_tid', 'subs_order_no' ), true ) ) {
