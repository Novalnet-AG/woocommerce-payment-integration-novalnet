...
class WC_Gateway_Novalnet_Cc extends WC_Novalnet_Abstract_Payment_Gateways {
	...
	public function payment_fields() {
		...
		novalnet()->helper()->load_template(
			...
			array_merge(
				$this->settings,
				array(
					'amount'   => wc_novalnet_formatted_amount( WC()->cart->total ),
	...
	public function validate_fields() {
		...
		if ( ( empty( $session ['wc-novalnet_cc-payment-token'] ) || wc_novalnet_check_isset( $session, 'wc-novalnet_cc-payment-token', 'new' ) ) && ! WC_Novalnet_Validation::validate_payment_input_field(
			...
			// Display message.
			$error = __( 'Your card details are invalid', 'woocommerce-novalnet-gateway' );
		} elseif ( ( ! empty( $session['novalnet_checkout_amount'] ) && ! empty( $session['novalnet_authenticated_amount'] ) ) && $session['novalnet_checkout_amount'] !== $session['novalnet_authenticated_amount'] ) {
			// Display message.
			$error = __( 'Transaction amount differs from the authenticated amount', 'woocommerce-novalnet-gateway' );
		}
		if ( ! empty( $error ) ) {
			...
			return $this->novalnet_redirect( wc_get_cart_url() );
		}
	}
