...
class WC_Gateway_Novalnet_Instalment_Sepa extends WC_Novalnet_Abstract_Payment_Gateways {
	...
	public function validate_fields() {
		...
		if ( in_array( strtoupper( $get_country ), array( 'CH', 'MC', 'SM', 'GB' ), true ) && empty( $session['novalnet_instalment_sepa_bic'] ) && ( wc_novalnet_check_isset( $session, 'wc-novalnet_instalment_sepa-payment-token', 'new' ) || empty( $session ['wc-novalnet_instalment_sepa-payment-token'] ) ) ) {
			// Display message.
			$this->display_info( __( 'Your account details are invalid', 'woocommerce-novalnet-gateway' ) );
			// Redirect to checkout page.
			return $this->novalnet_redirect();
