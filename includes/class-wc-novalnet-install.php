...
class WC_Novalnet_Install
{
    ...
    public static function update()
    {
        ...
        if (version_compare($current_db_version, NOVALNET_VERSION, '!=')) {
            ...
            if (version_compare($current_db_version, '13.0.0', '<')) {

                $settings = get_option('woocommerce_novalnet_ideal_settings');
            
                if (!empty($settings)) {
            
                    // Update English
                    $settings['title_en'] = 'iDEAL | Wero';
                    $settings['description_en'] =
                        'You will be redirected to iDEAL | Wero. Please don’t close or refresh the browser until the payment is completed';
            
                    // Update German
                    $settings['title_de'] = 'iDEAL | Wero';
                    $settings['description_de'] =
                        'Sie werden zu iDEAL | Wero weitergeleitet. Um eine erfolgreiche Zahlung zu gewährleisten, darf die Seite nicht geschlossen oder neu geladen werden, bis die Bezahlung abgeschlossen ist.';
            
                    if (!isset($settings['debug'])) {
                        $settings['debug'] = 'yes';
                    }
            
                    update_option('woocommerce_novalnet_ideal_settings', $settings);
                }
            }
