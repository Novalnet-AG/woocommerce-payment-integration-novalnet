        ...
        paymentObject = {
            ...
            label: Object(wpElement.createElement)((e) => {
                return Object(wpElement.createElement)(
                    e.components.PaymentMethodLabel,
                    { icon: '', text: paymentTitle },
                );
            }, null),
                ...
                wpElement.useEffect( () => {
                    const unsubscribe = onPaymentProcessing( async () => {
                        ...
                        if ( novalnet_guaranteed_sepa_iban.length != 0 ) {
                            ...
                            if ( null !== dateOfBirth ) {
                                const novalnet_guaranteed_sepa_dob = document.getElementById( paymentMethodId + '_dob' ).value;
                                if ( novalnet_guaranteed_sepa_dob.length != 0 ) {
                                    paymentMethodData[ paymentMethodId + '_sepa_dob' ] = novalnet_guaranteed_sepa_dob;
                                }
                            }
                            return {
            ...
            supports:{
                showSavedCards: ( null == paymentMethodData ? false : paymentMethodData.enableTokenization ),
                showSaveOption: ( null == paymentMethodData ? false : paymentMethodData.enableTokenization ),
