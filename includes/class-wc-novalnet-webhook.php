...
class WC_Novalnet_Webhook {
	...
	public function handle_payment() {
		...
		&& '1' === novalnet()->helper()->novalnet_get_wc_order_meta( $this->wc_order, '_novalnet_booking_ref_order' ) ) {
	...
	public function handle_subscription_reactivate() {
		...
		novalnet()->helper()->novalnet_update_wc_order_meta( $this->wcs_order, '_schedule_next_payment', $this->event_data ['subscription']['next_cycle_date'] );
		novalnet()->helper()->novalnet_update_wc_order_meta( $this->wcs_order, '_schedule_end', $end_date );
		novalnet()->helper()->novalnet_update_wc_order_meta( $this->wcs_order, '_schedule_trial_end', $trial_date );
		$this->wcs_order->save();
	...
	public function handle_subscription_update() {
		...
		$recurring_date   = gmdate( 'Y-m-d', strtotime( $this->wcs_order->get_date( 'next_payment' ) ) );
		...
		if ( ( ! empty( $this->event_data ['subscription']['amount'] ) && (int) $recurring_amount !== (int) $this->event_data ['subscription']['amount'] ) || ( $recurring_date !== $next_cycle_date ) ) {
			...
			if ( ! empty( $next_cycle_date ) ) {
				novalnet()->helper()->novalnet_update_wc_order_meta( $this->wcs_order, '_schedule_next_payment', gmdate( 'Y-m-d H:i:s', strtotime( $next_cycle_date ) ), true );
			}
		}
