...
class WC_Novalnet_Webhook
{
    ...
    public function handle_transaction_refund()
    {
        if (! empty($this->event_data['transaction']['refund']['amount'])) {
            ...
            if (is_wp_error($refund)) {
                ...
                $message = PHP_EOL . sprintf(__('Payment refund failed for the order: %1$s due to: %2$s', 'woocommerce-novalnet-gateway'), $this->wc_order_id, $refund->get_error_message());
                // If there is an error with the payment refund, we can log the data for future reference.
                novalnet()->helper()->debug($message, $this->wc_order_id);
            }
        }
    }
    ...
    public function validate_checksum()
    {
        ...
        if (empty($this->payment_access_key)) {
            $this->display_message(array( 'message' => 'Payment access key is not configured. Unable to authenticate notification.' ));
            return;
        }
