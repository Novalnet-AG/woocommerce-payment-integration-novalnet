...
class WC_Novalnet_Webhook {
	...
	public function handle_subscription_update() {
		...
		$update_type = isset( $this->event_data ['subscription']['update_type'] ) ? $string = implode(', ', $this->event_data ['subscription']['update_type']): '';

		if ( ! empty( $update_type ) ) {
			if ( ( 'RENEWAL_AMOUNT' === $update_type && ! empty( $this->event_data ['subscription']['amount'] ) ) || ( 'RENEWAL_DATE' === $update_type ) ) {
			...
			if ( 'PAYMENT_DATA' === $update_type && ! empty( $this->event_data ['transaction'] ['payment_type'] ) ) {
	...
	public function send_notification_mail( $comments ) {
		...
		if ( ! empty( $order_id ) ) {
			$message .= wc_novalnet_format_text( sprintf( __( ' <b>[Order #%s] (%s)</b>' . PHP_EOL . PHP_EOL, 'woocommerce-novalnet-gateway' ), $order_id, wc_novalnet_formatted_date() ) );
		}
		$message .= $comments ['message'];
		wc_novalnet_send_mail( WC_Novalnet_Configuration::get_global_settings( 'callback_emailtoaddr' ), $mail_subject, $message );
	}
