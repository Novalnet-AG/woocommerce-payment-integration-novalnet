...
class WC_Novalnet_Webhook {
	...
	public function handle_subscription_cancel() {
		...
		if( ! empty ( $subs_cancel_reason ) ) {
			$this->event_data ['subscription']['reason'] = $subs_cancel_reason;
		}
	 ...
	 * @since 12.4.0
	 */
	public function handle_payment_reminder( $remainder_count ) {
		/* translators: %s: reminder count */
		$this->response ['message'] = sprintf( __( 'Payment Reminder %1$s has been sent to the customer.', 'woocommerce-novalnet-gateway' ), $remainder_count );
	}
	...
	public function handle_transaction_update() {
		...
		if ( in_array( $this->event_data['transaction']['status'], array( 'PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED' ), true ) ) {
			...
			WC()->mailer()->get_emails()['WC_Email_New_Order']->trigger( $this->wc_order->get_id() );
