...
class WC_Novalnet_Webhook {
	...
	public function __construct() {
		...
		if ( 'RENEWAL' === $this->event_type && ! WC_Novalnet_Validation::is_success_status( $this->event_data ) && ! empty( $this->event_data['result']['status_text'] ) ) {
			update_post_meta( $this->wc_order_id, '_subs_cancelled_reason', $this->event_data['result']['status_text'] );
		}
	...
	public function handle_subscription_cancel() {
		...
		$subs_cancel_reason = get_post_meta( $this->wc_order_id, '_subs_cancelled_reason', true );
		...
		if ( ! empty( $subs_cancel_reason ) ) {
			...
			delete_post_meta( $this->wc_order_id, '_subs_cancelled_reason' );
		}
	...
	public function handle_subscription_update() {
		...
		$recurring_date   = gmdate( 'Y-m-d', strtotime( get_post_meta( $this->wcs_order_id, '_schedule_next_payment', true ) ) );
	...
	public function handle_renewal() {
		...
		if ( in_array( $this->event_data['transaction']['status'], array( 'CONFIRMED', 'PENDING' ), true ) ) {
			...
			update_post_meta( $this->wcs_order->get_id(), '_schedule_next_payment', gmdate( 'Y-m-d H:i:s', strtotime( $next_cycle_date ) ) );

			$total_length = apply_filters( 'novalnet_get_order_subscription_length', $this->wcs_order );

			$coupon_discount_payment_count = get_post_meta( $this->wc_order_id, '_novalnet_wcs_number_payments', true );
			...
			if ( ! empty( $total_length ) && $related_orders >= $total_length ) {
				...
				$tid = novalnet()->db()->get_subs_data_by_order_id( $this->wcs_order->get_parent_id(), $this->wcs_order->get_id(), 'tid', false );
				if ( empty( $tid ) ) {
					$tid = novalnet()->db()->get_transaction_details( $this->wc_order_id, '', $this->wcs_order->get_id() );
				}
				...
				/* translators: %s: tid */
				$this->subscription_cancel_note = PHP_EOL . PHP_EOL . wc_novalnet_format_text( sprintf( __( 'Subscription has been cancelled since the subscription has exceeded the maximum time period for the TID: %s', 'woocommerce-novalnet-gateway' ), $tid ) );
			...
			if ( version_compare( get_option( 'woocommerce_subscriptions_active_version' ), '4.0.0', '<' ) ) {
	...
	public function handle_credit() {
		if ( 'ONLINE_TRANSFER_CREDIT' === $this->event_data['transaction']['payment_type'] ) {
			...
			if ( in_array( $this->event_data['transaction']['payment_type'], array( 'INVOICE_CREDIT', 'CASHPAYMENT_CREDIT', 'MULTIBANCO_CREDIT' ), true ) ) {
				...
				$subscriptions = wcs_get_subscriptions_for_order( $this->wc_order_id );
	...
	public function handle_transaction_update() {
		...
		if ( in_array( $this->event_data['transaction']['status'], array( 'PENDING', 'ON_HOLD', 'CONFIRMED', 'DEACTIVATED' ), true ) ) {
			...
			if ( ! empty( $transaction_comments ) ) {
				$customer_given_note = get_post_meta( $this->wc_order->get_id(), '_nn_customer_given_note', true );
	...
	public function update_recurring_order_amount() {
		$tid = novalnet()->db()->get_subs_data_by_order_id( $this->wcs_order->get_parent_id(), $this->wcs_order->get_id(), 'tid', false );
		if ( empty( $tid ) ) {
			$tid = novalnet()->db()->get_entry_by_order_id( $this->wcs_order->get_parent_id(), 'tid' );
		}
		$parameters = array(
			'subscription' => array(
		...
		$server_response = novalnet()->helper()->submit_request( $parameters, novalnet()->helper()->get_action_endpoint( 'subscription_update' ), array( 'post_id' => $this->wcs_order->get_id() ) );
	}
