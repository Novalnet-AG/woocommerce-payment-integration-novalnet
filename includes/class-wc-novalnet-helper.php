...
class WC_Novalnet_Helper {
	...
	public function form_amount_transfer_comments( $input, $reference = true ) {
		...
		if ( ! empty( $input['instalment']['cycle_amount'] ) ) {
			...
			$amount_text = 'instalment cycle amount';
		}
		if ( in_array( $input['transaction']['status'], array( 'CONFIRMED', 'PENDING' ), true ) && ! empty( $input ['transaction']['due_date'] ) ) {
			...
			$comments = PHP_EOL . PHP_EOL . sprintf( __( 'Please transfer the %1$s of %2$s to the following account on or before %3$s', 'woocommerce-novalnet-gateway' ), $amount_text, wc_novalnet_shop_amount_format( $order_amount ), wc_novalnet_formatted_date( $input ['transaction']['due_date'] ) ) . PHP_EOL . PHP_EOL;
		} else {
			/* translators: %s: amount*/
			$comments = PHP_EOL . PHP_EOL . sprintf( __( 'Please transfer the %1$s of %2$s to the following account.', 'woocommerce-novalnet-gateway' ), $amount_text, wc_novalnet_shop_amount_format( $order_amount ) ) . PHP_EOL . PHP_EOL;
		}
