...
class WC_Novalnet_Helper {
	...
	public function prepare_transaction_table_data( $wc_order, $payment_id, $server_response ) {
		...
		if ( novalnet()->get_supports( 'zero_amount_booking', $payment_id ) && '1' === $this->novalnet_get_wc_order_meta( $wc_order, '_novalnet_booking_ref_order' ) ) {
			$booking_data = array(
	...
	public function register_payment_script( $payment_id, $novalnet_utility = false, $novalnet_payment = false ) {
		...
		$deps              = array( 'jquery', 'react', 'wc-blocks-registry', 'wc-settings', 'wp-element', 'wp-i18n', 'wp-polyfill' );
		...
		wp_register_script(
			...
			novalnet()->plugin_url . '/includes/wc-blocks/assets/block-novalnet-elements.js',
			$deps,
			NOVALNET_VERSION,
			true
		);
		...
		wp_register_script(
			$script_handle,
			novalnet()->plugin_url . '/includes/wc-blocks/assets/block-' . $script_payment_id . '.js',
	...
	public function is_cart_block_default() {
		if ( class_exists( 'Automattic\WooCommerce\Blocks\Utils\CartCheckoutUtils' ) && CartCheckoutUtils::is_cart_block_default() ) {
			return true;
		}
		return false;
	}
