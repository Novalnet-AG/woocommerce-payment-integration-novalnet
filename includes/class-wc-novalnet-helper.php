...
class WC_Novalnet_Helper {
	...
	public function submit_request( $request, $url, $args = array() ) {
		...
		if ( ! empty( $args['access_key'] ) ) {
			...
			$this->debug( "REQUEST: {$url} - $json_request", $args['post_id'], $override_log_setting );
			...
			if ( is_wp_error( $response ) ) {
				...
				$this->debug( "RESPONSE: {$url} - " . $response['body'], $args['post_id'], $override_log_setting );
	...
	public function form_comments( $data, $is_error = false ) {
		...
		if ( WC_Novalnet_Validation::is_success_status( $data ) && in_array( $data['transaction']['payment_type'], array( 'GOOGLEPAY', 'APPLEPAY' ), true ) ) {
			...
			$comments .= sprintf( __( 'Your order was successfully processed using %1$s(%2$s ****%3$s)', 'woocommerce-novalnet-gateway' ), $payment, strtolower( $data['transaction']['payment_data']['card_brand'] ), $data['transaction']['payment_data']['last_four'] ) . PHP_EOL;
		}
