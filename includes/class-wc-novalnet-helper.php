...
class WC_Novalnet_Helper
{
    ...
    public function prepare_payment_comments($data, $wc_order_id = false)
    {
        ...
        }elseif(!empty($data['instalment']['prepaid'])){
			if (empty($wc_order_id) && (!empty($data['custom']['nn_order_id']) || !empty($data['custom']['inputval5']))) {
                $wc_order_id = $data['custom']['nn_order_id'] ?? $data['custom']['inputval5'];
            }
			if($wc_order_id){
				$wc_order = wc_get_order($wc_order_id);
				$this->novalnet_delete_wc_order_meta($wc_order, '_novalnet_qr_code');
                 $this->novalnet_delete_wc_order_meta($wc_order, '_novalnet_epc_qr_code');
			}
		}
    ...
    public function form_comments($data, $is_error = false)
    {
        ...
        if (! empty($data['transaction']['tid'])) {
            ...
            /* translators: %s: TID */
            $comments .= sprintf(__("Novalnet transaction ID: %s  \n", 'woocommerce-novalnet-gateway'), $data['transaction']['tid']);
            if (! empty($data['transaction']['test_mode'])) {
                $comments .= __(" Test order \n", 'woocommerce-novalnet-gateway');
            }
        }
        if ($is_error) {
