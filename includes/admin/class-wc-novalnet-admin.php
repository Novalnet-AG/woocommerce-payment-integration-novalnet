...
class WC_Novalnet_Admin extends WC_Settings_API {
	...
	public function status_change_update() {
		...
		if ( ! empty( novalnet()->request['action'] ) && 'woocommerce_mark_order_status' === novalnet()->request['action'] && ( 'completed' === novalnet()->request['status'] || 'processing' === novalnet()->request['status'] ) ) {
			$post_id             = novalnet()->db()->get_post_id_by_order_number( novalnet()->request['order_id'] );
			$wc_order            = wc_get_order( $post_id );
			$transaction_details = novalnet()->db()->get_transaction_details( $post_id, 'tid' );
			if ( ! empty( $transaction_details ) && 'ON_HOLD' === $transaction_details['gateway_status'] ) {
				$action = 'transaction_capture';
				$tid    = novalnet()->db()->get_entry_by_order_id( $post_id, 'tid' );
				...
				$response   = novalnet()->helper()->submit_request( $parameters, novalnet()->helper()->get_action_endpoint( $action ), array( 'post_id' => $post_id ) );
		...
		if ( ! empty( $wc_order_id ) ) {
			...
			if ( in_array( $nn_post_type, array( 'shop_order', 'subscription_order' ), true ) ) {
				...
				if ( WC_Novalnet_Validation::check_string( $payment_method ) ) {
					...
					} elseif ( novalnet()->get_supports( 'zero_amount_booking', $payment_method ) && '1' === get_post_meta( $wc_order_id, '_novalnet_booking_ref_order', 1 ) ) {
						include_once 'meta-boxes/class-wc-novalnet-meta-box-amount-book.php';
						add_action( 'woocommerce_process_shop_order_meta', array( 'WC_Novalnet_Meta_Box_Amount_Book', 'save' ), 1, 1 );
	...
	/**
	 * Add order amount booking meta box.
	 *
	 * @since 12.6.0
	 */
	public function add_amount_book_boxes() {
		add_meta_box( 'wc-novalnet-amount-book', _x( 'Zero amount booking', 'meta box title', 'woocommerce-novalnet-gateway' ), 'WC_Novalnet_Meta_Box_Amount_Book::output', 'shop_order', 'side', 'default' );
	}
	...
	public function handle_amount_update( $wc_order_id ) {
		...
		if ( ( 'shop_order' === get_post_type( $wc_order_id ) ) && ( wc_novalnet_check_isset( novalnet()->request, 'action', 'woocommerce_save_order_items' ) || wc_novalnet_check_isset( novalnet()->request, 'action', 'woocommerce_calc_line_taxes' ) || wc_novalnet_check_isset( novalnet()->request, 'action', 'novalnet_wc_order_recalculate_success' ) ) ) {
	...
	public function handle_status_change( $wc_order_id, $old_status, $new_status, $wc_order ) {
		...
		if ( WC_Novalnet_Validation::check_string( $wc_order->get_payment_method() ) && wc_novalnet_check_isset( novalnet()->request, 'post_type', 'shop_order' ) ) {
			...
			if ( 'cancelled' === $new_status && in_array( $gateway_status, array( 'CONFIRMED', 'PENDING' ), true ) ) {
				...
				if ( ! in_array( $transaction_details['payment_type'], array( 'novalnet_invoice', 'novalnet_prepayment', 'novalnet_barzahlen', 'novalnet_multibanco' ), true ) ) {
					$add_refund_log = true;
				} elseif ( $transaction_details['callback_amount'] > 0 ) {
					$add_refund_log = true;
				}
	...
	public function wc_admin_shop_order_novalnet_payment_fields( $order ) {
		...
		if ( 'shop_order' === $post_type && 'auto-draft' === $order_data['status'] ) {
			$available_gateways = WC()->payment_gateways->get_available_payment_gateways();
			?>
	...
	public function novalnet_wc_shop_order_customer_note_check( $wc_order_data_object ) {
		if ( is_admin() && isset( novalnet()->request['post_type'] ) && 'shop_order' === novalnet()->request['post_type'] && 'editpost' === novalnet()->request['action'] && 'send_order_details_admin' === novalnet()->request['wc_order_action'] ) {
