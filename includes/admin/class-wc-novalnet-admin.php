...
class WC_Novalnet_Admin extends WC_Settings_API {
	...
	public function handle_amount_update( $wc_order_id ) {
		...
		if ( 'shop_order' === get_post_type( $wc_order_id ) && wc_novalnet_check_isset( novalnet()->request, 'action', 'woocommerce_save_order_items' ) ) {
					...
					if ( in_array( $gateway_status, array( 'ON_HOLD', 'PENDING' ), true ) ) {
						...
						if ( novalnet()->db()->get_entry_by_order_id( $wc_order_id, 'amount' ) !== $updated_amount ) {
							...
							if ( WC_Novalnet_Validation::is_success_status( $response ) ) {
								...
								novalnet()->helper()->update_comments( $wc_order, sprintf( __( 'Amount update failed due to: %s' ), wc_novalnet_response_text( $response ) ) );
							}
						}
					}
				}
			...
			if ( ! empty( $action ) ) {
				...
				if ( ! empty( $tid ) ) {
					...
					if ( WC_Novalnet_Validation::is_success_status( $response ) ) {
						...
						} elseif ( 'transaction_refund' === $action ) {
						...
						} elseif ( 'instalment_cancel' === $action ) {

							/* translators: %s: refund_tid */
							$message = sprintf( __( 'Instalment has been cancelled for the TID: %s', 'woocommerce-novalnet-gateway' ), $response ['transaction']['refund']['tid'] );
						}
						// Update transaction details.
						novalnet()->db()->update(
							$update_info,
