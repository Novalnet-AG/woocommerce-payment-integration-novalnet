...
class WC_Novalnet_Admin extends WC_Settings_API {
	...
	public function status_change_update() {
		...
		if( ! empty( novalnet()->request['action'] ) && novalnet()->request['action'] == 'woocommerce_mark_order_status' && ( 'completed' == novalnet()->request['status'] || 'processing' == novalnet()->request['status'] ) ) {
			...
			if( ! empty( $transaction_details ) && $transaction_details['gateway_status'] = 'ON_HOLD') {
	...
	public function handle_status_change( $wc_order_id, $old_status, $new_status, $wc_order ) {
		...
		if ( WC_Novalnet_Validation::check_string( $wc_order->get_payment_method() ) && wc_novalnet_check_isset( novalnet()->request, 'post_type', 'shop_order' ) ) {
			...
			if ( 'cancelled' === $new_status && in_array( $gateway_status, array( 'CONFIRMED', 'PENDING' ), true ) ) {
				...
				if( ! in_array( $transaction_details['payment_type'], array('novalnet_invoice', 'novalnet_prepayment', 'novalnet_barzahlen', 'novalnet_multibanco') ) )  {
					$add_refund_log = true;
				} else if ( $transaction_details['callback_amount'] > 0 ) {
					$add_refund_log = true;
				}
