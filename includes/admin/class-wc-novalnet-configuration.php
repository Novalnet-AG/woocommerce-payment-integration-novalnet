...
class WC_Novalnet_Configuration extends WC_Settings_API {
		...
		if ( novalnet()->get_supports( 'instalment', $payment_type ) ) {
			...
			$form_fields ['guarantee_payment_title'] = array(
				...
				'description' => sprintf(
					...
					sprintf( __( 'Allowed B2B countries: European Union', 'woocommerce-novalnet-gateway' ) ),
	...
	public static function guarantee( &$form_fields, $payment_type = '', $is_guarantee = false ) {
		...
		if ( $is_guarantee ) {
			...
			$form_fields['force_normal_payment'] = array(
				...
				'default'     => 'no',
				'description' => $note,
				'desc_tip'    => __( 'Even if payment guarantee is enabled, payments will still be processed as non-guarantee payments if the payment guarantee requirements are not met. Review the requirements under "Enable Payment Guarantee" in the Installation Guide.', 'woocommerce-novalnet-gateway' ),
			);
		}
		...
		return apply_filters(
			...
			array(
				...
				array(
					...
					'desc'              => '<br/>' . __( 'Get your Product activation key from the <a href="https://admin.novalnet.de" target="_blank">Novalnet Admin Portal</a> PROJECT > Choose your project > Shop Parameters > API Signature (Product activation key) ', 'woocommerce-novalnet-gateway' ),
				...
				array(
					...
					'desc'              => '<br/>' . __( 'Get your Payment access key from the <a href="https://admin.novalnet.de" target="_blank">Novalnet Admin Portal</a> PROJECT > Choose your project > Shop Parameters > Payment access key', 'woocommerce-novalnet-gateway' ),
				...
				array(
					...
					'desc'     => sprintf( __( 'Find the Novalnet payment events log in this path:  <code>%s.txt</code>', 'woocommerce-novalnet-gateway' ), wc_get_log_file_path( 'woocommerce-novalnet-gateway' ) ),
					'desc_tip' => true,
				),
				array(
					'type' => 'sectionend',
	...
	public static function tokenization( &$form_fields, $payment_type ) {
		...
		if ( 'novalnet_paypal' === $payment_type ) {
			$default         = 'no';
			$additional_info = __( 'In order to use this option you must have billing agreement option enabled in your PayPal account. Please contact your account manager at PayPal.', 'woocommerce-novalnet-gateway' );
		}
	...
	public static function basic( &$form_fields, $payment_type ) {
		...
		$payment_configuration = 'woocommerce_' . $payment_type . '_settings';
		$option_value          = get_option( $payment_configuration );
		$option_value['lang']  = wc_novalnet_shop_language();

		update_option( 'woocommerce_' . $payment_type . '_settings', $option_value );
	...
	public static function get_payment_text( $payment_type ) {
		...
		$payment_text = array(
			'novalnet_cc'                 => array(
				...
				'description_en' => 'The amount will be debited from your credit/debit card',
				'description_de' => 'Der Betrag wird Ihrer Kredit-/Debitkarte belastet',
