	...
	public static function basic( &$form_fields, $payment_type ) {
		...
		if ( ! in_array( $payment_type, array( 'novalnet_applepay', 'novalnet_googlepay' ), true ) ) {
			$form_fields ['enable_for_methods'] = array(
				...
				'css'               => 'width: 400px;',
				'default'           => '',
				'description'       => sprintf( __( 'If %1$s is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'woocommerce' ), $payment_text ['title_en'] ),
		...
		$form_fields ['enable_for_virtual'] = array(
			...
			'label'   => sprintf( __( 'Accept %1$s if the order is virtual', 'woocommerce' ), $payment_text ['title_en'] ),
			'type'    => 'checkbox',
			'default' => 'yes',
		);
