...
class WC_Payment_Token_Novalnet extends WC_Payment_Token {
	...
	/**
	 * Returns the ACH Account number.
	 *
	 * @since  12.7.0
	 * @param  string $context What the value is for. Valid values are view and edit.
	 * @return string Account number
	 */
	public function get_ach_account_number( $context = 'view' ) {
		return $this->get_prop( 'ach_account_number', $context );
	}
	 ...
	 */
	public function set_ach_account_number( $account_number ) {
		$this->set_prop( 'ach_account_number', $account_number );
	}
	...
	public function get_display_name( $deprecated = '' ) {
		...
		} elseif ( 'novalnet_ach' === $this->get_gateway_id() ) {
			$display = sprintf(
				/* translators: Account no */
				__( 'Account No %s', 'woocommerce-novalnet-gateway' ),
				$this->get_ach_account_number()
			);
		}
		return $display;
	}
	...
	public static function saved_payment_methods_list_item( $item, $payment_token ) {
		...
		if ( 'novalnet_cc' === $payment_token->get_gateway_id() ) {
			...
			$item['expires']         = 'N/A';
		} elseif ( 'novalnet_ach' === $payment_token->get_gateway_id() ) {
			$item['method']['last4'] = substr( $payment_token->get_ach_account_number(), -4 );
			$item['method']['brand'] = 'ACH Account No';
			$item['expires']         = 'N/A';
		}
		return $item;
	}
	...
	public function delete_duplicate_tokens( $data, $payment_type ) {
		...
		foreach ( $users_tokens as $user_token ) {
			if ( in_array( $payment_type, array( 'novalnet_sepa', 'novalnet_guaranteed_sepa', 'novalnet_instalment_sepa' ), true ) && in_array( $user_token->get_gateway_id(), array( 'novalnet_sepa', 'novalnet_guaranteed_sepa', 'novalnet_instalment_sepa' ), true ) && wc_novalnet_check_isset( $data, 'iban', $user_token->get_iban() ) ) {
				...
				} elseif ( 'novalnet_ach' === $payment_type && wc_novalnet_check_isset( $data, 'ach_account_number', $user_token->get_ach_account_number() ) ) {
					WC_Payment_Tokens::delete( $user_token->get_id() );
				}
			}
		}
	...
	public function store_token_data( $payment_type, $data ) {
		if ( 'novalnet_cc' === $payment_type ) {
			...
			$this->set_ach_account_number( $data['account_number'] );
		}
	}
