	...
	wc_novalnet_admin = {
		...
		init : function () {
			...
			if ('undefined' !== typeof wc_novalnet_admin_supported_payment) {
				jQuery('#_payment_method >option').each(function() {
					if( jQuery.inArray(this.value, wc_novalnet_admin_supported_payment) == -1 ) {
						if( this.value.indexOf("novalnet") != -1 ) {
							jQuery('#_payment_method option[value="'+this.value+'"]').hide();
						}
					}
					if( this.value.indexOf("novalnet_guaranteed") != -1 ) {
				...
				$( '#_billing_country , #_billing_company' ).on('change', function() {
					var billing_country = $('#_billing_country').val();
						...
						if( jQuery.inArray( billing_country , check_list ) == -1 ) {						
							jQuery('#_payment_method >option').each(function() {
								if( this.value.indexOf("novalnet_guaranteed") != -1 ) {
				...
				'change', wc_novalnet_admin.validate_form_fields);
				$( document ).on(
					'submit',
					...
					function( event ) {
						...
						if(payment_method == '' || payment_method.length == 0) {
							if ($("select[id='_payment_method'] option[value*='novalnet_']").length > 0) {
				                wc_novalnet.show_error( '#novalnet_admin_order_error', wc_novalnet_admin_data.payment_method_empty );
				                event.preventDefault();
								event.stopImmediatePropagation();
				            }
						}
						if( jQuery.inArray(payment_method, wc_novalnet_admin_supported_payment) != -1 ) {
			...
			if(( 'auto-draft' != $( '#original_post_status' ).val()) && 'shop_subscription' != $( '#post_type' ).val()) {
				var processed_payment = $( '#_payment_method' ).val();
				$( '#_payment_method' ).on( 'change' , function(event) {
					if( $(this).val().indexOf('novalnet') != -1 || processed_payment.indexOf('novalnet') != -1) {
		...
		validate_form_fields: function() {
			...
			if( ( jQuery.inArray(payment_method, wc_novalnet_admin_supported_payment) != -1 ) ) {
				...
				if( payment_method.indexOf( 'novalnet_guaranteed' ) != -1 ) {
					jQuery( '#' + payment_method + '_dob_field' ).show();
				    if(NovalnetUtility.isValidCompanyName( $( '#_billing_company' ).val() ) && jQuery( '#wc_shop_order_admin_' + payment_method ).attr('allow_b2b') == 'yes' ) {
				...
				if ( transaction_id.length != '0' ) {
					if(transaction_id.length == '17' && transaction_id.match(/^\d+$/)) {
						jQuery( '.wc_shop_admin_order_novalnet_method' ).hide();								
					} else {
